...
def rescale_image(image, height, width, out_file):
    if height < width:
        command = '%s -i %s -vf pad=%s:%s:0:oh/2-ih/2 %s -y 2' % (FFMPEG, image, width, height, out_file)
    else:
        command = '%s -i %s -vf pad=%s:%s:0:ow/2-iw/2 %s -y 2' % (FFMPEG, image, width, height, out_file)
...
def trim_video_by_seconds(video_file, start, end, out_file):
    command = '%s -ss %s -i %s -c copy -t %s %s 2' % (FFMPEG, start, video_file, end, out_file)
    os.system(command)
...
def mp3_to_aac(mp3_file, aac_file):
    command = '%s -i %s -c:a libfdk_aac %s 2' % (FFMPEG, mp3_file, aac_file)
    os.system(command)


def webm_to_mp4(webm_file, mp4_file):
    command = '%s -i %s -qscale 0 %s 2' % (FFMPEG, webm_file, mp4_file)
    os.system(command)
